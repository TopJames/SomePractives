...
(function(win, $){
	...
	var videoPlayer = function(dom,src){
		...
		this.isAudioPlayer = function (isAudio) {
			if(isAudio != undefined && isAudio != null)
            	this._audioPlayer = isAudio;
		...
		this.init = function(){
			...
			this._adpaterHLS();
			var mthis = this;
			this.getPlayer().oncanplay = function(){
				
			}
			this.getPlayer().onwaiting = function(){
				//console.log("等待资源中");
			}
			this.getPlayer().onloadedmetadata = function(){
				var totalTime = Math.ceil(this.duration);
				var showTime = mthis._formaterTime(totalTime);
				if(mthis.storage.initSeekTo){
					mthis.seekTo(this.duration + mthis.storage.initSeekTo > 0 ? this.duration + mthis.storage.initSeekTo : 0); 
				}
		...
		this.seekTo = function(time,buildStatus){
			if(this.storage.initSeekTo)
				this.storage.initSeekTo = undefined;
			if(time<0){
		...
		this.voice = function(voice){
			...
			if(!voice && voice != 0){
				...
				//this._hideTips();
				player.volume = parseFloat(voice);
				$(this.dom).find(("*[playerClass='voiceCurLine']")).css("top", (100 - parseFloat(voice) * 100 ) + 'px');
				$(this.dom).find("*[playerClass='playerVoiceIcon']").css("top", (100 - parseFloat(voice) * 100 ) - 10 + 'px')
		...
		this._showPlayStatus = function(){
			...
			$(mthis.dom).find("*[playerClass='beginTimeFont']").text(this._formaterTime(curTime));
		...
		this._adpaterHLS = function(){
			...
			if(src && 
				...
				$(mthis.dom).find("video").first().find("source").attr("type","application/x-mpegURL");
				$(mthis.dom).find("video").addClass("video-js vjs-default-skin");
				var _ttplayer = videojs(mthis.videoCtrlId);
				_ttplayer.src({
		            src: $(mthis.dom).find("video").first().find("source").attr("src"),
		            type: 'application/x-mpegURL'
		        });

                this.m3u8Obj = _ttplayer;
				
				var eles = $(mthis.dom).find("video").siblings().hide();
				for(var i=0; i<eles.length; i++){
					//$(eles[i]).hide();
				}
				
				mthis.videoCtrlId = $(mthis.dom).find("video").attr("id");
			}
		}

        this._unbindListener = function(){
            $(this.dom).unbind("mouseover");
            $(this.dom).unbind("mouseout");
            $(this.dom).unbind("mouseleave");
		...
		this._bindListener = function(){
			...
			$(this.dom).find("*[playerClass='reduceSpeedIcon']").unbind("click").click(function(e){
				...
				else if(playRate > 2 && playRate <= 4)
					playerCtrl.playbackRate = 2;
				else if(playRate > 4 && playRate <= 8)
			...
			$(this.dom).find("*[playerClass='raiseSpeedIcon']").unbind("click").click(function(e){
				...
				if(playRate == 1)
					...
					mthis._showTips("当前播放速度已是浏览器最大支持的播放速度", $(mthis.dom).height()-120, mthis._getMouseX(e.pageX));
				}
				mthis._showPlayStatus();
			});
	...
	var _VIDEOPLAYERHTML_SKIN = {
		...
		DEFAULT : function(dom){
			...
			return {
				...
				playerLinerContainer: {"background":"RGBA(0,0,0,0.1)","position": "absolute","width":"100%","height":"30px","margin-top":"-85px","text-align":"center","line-height":"30px"},
				...
				controllerDiv : {"background":"RGBA(0,0,0,0.1)", "position": "absolute", "width": "100%", "height": "50px", "margin-top": "-55px", "text-align":"center", "line-height":"50px"},
				...
				tipsShowDom : { "padding":"5px", "background":"RGBA(0,0,0,0.5)","position": "absolute","color":"RGB(255,136,0)","z-index":'999',"font-size":"14px","font-weight":"bold"},
				playRateShow: {"display":"none", "position": "absolute","font-size":"18px","color":"RGB(255,136,0)","top":"10px","left":"10px","z-index": "999","cursor":"default"},
			...
			'<div>'+
				...
				'<div playerClass="playerOuter">'+
					'<div playerClass="playerContainer" adapaterScreen>'+
						...
						'<video id="${playerId}" preload="none" playerClass="playerVideo">'+
							'<source src="${videoUrl}">'+
						'</video>'+
